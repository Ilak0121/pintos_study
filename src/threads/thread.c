...
bool 
time_compare (struct list_elem* first,struct list_elem* second ,void* aux){
...
//ordering func for priority
bool 
priority_comp (struct list_elem* first,struct list_elem* second, void*aux){
    struct thread *t1 = list_entry(first,struct thread, elem);
    struct thread *t2 = list_entry(second,struct thread, elem);
...
thread_wait (int64_t ticks){
    ...
    list_insert_ordered(&wait_list,&t->elem,time_compare,0);//time_compare,0);
    thread_block();
...
int64_t 
awake_ticks(){
    struct list_elem *wait_e = list_begin(&wait_list); 
    struct thread *t = list_entry(wait_e,struct thread,elem);
    return t->wait_time;
}
